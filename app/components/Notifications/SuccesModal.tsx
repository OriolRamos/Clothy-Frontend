...
interface SuccessModalProps {
    ...
    text?: string; // El text del modal, per defecte és '¡Usuario guardado correctamente!'
    duration?: number; // Duració del modal en segons, per defecte és 5 segons
}

const SuccessModal: React.FC<SuccessModalProps> = ({ isOpen, onClose, text = '¡Usuario guardado correctamente!', duration = 5 }) => {
    useEffect(() => {
    ...
    return (
        <div className="fixed top-16 left-1/2 transform -translate-x-1/2 z-50 pointer-events-none">
            <div
                className="bg-green-400 bg-opacity-90 text-white px-6 py-2 rounded-lg shadow-md w-72 text-center pointer-events-auto"
